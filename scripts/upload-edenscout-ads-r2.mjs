#!/usr/bin/env node
// Upload round-2 EdenScout ads to Google Drive at
//   EdenRadar/Ads/edenscout-awareness-2026-05-round-2/
// Talks directly to the Replit Google Drive connector token (same path as
// server/lib/googleDriveClient.ts) so we don't need Supabase login.
...
async function main() {
  ...
  for (const name of names) {
    ...
    // Idempotent: if a file by the same name exists, replace it so re-runs
