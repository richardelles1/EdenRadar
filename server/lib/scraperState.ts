...
export async function recordScraperSuccess(institution: string, newCount: number, rawCount?: number): Promise<void> {
  try {
    await db.execute(sql`
      INSERT INTO scraper_health (institution, consecutive_failures, last_failure_reason, last_failure_at, last_success_at, backoff_until, last_success_new_count, last_success_raw_count, updated_at)
      VALUES (${institution}, 0, NULL, NULL, NOW(), NULL, ${newCount}, ${rawCount ?? null}, NOW())
      ON CONFLICT (institution) DO UPDATE SET
        ...
        last_success_raw_count = ${rawCount ?? null},
        updated_at = NOW()
    `);
  } catch (err: any) {
...
export async function updateScraperHealth(institution: string, success: boolean, failureReason?: string, newCount?: number, rawCount?: number): Promise<void> {
  try {
    if (success) {
      await db.execute(sql`
        INSERT INTO scraper_health (institution, consecutive_failures, last_failure_reason, last_failure_at, last_success_at, backoff_until, last_success_new_count, last_success_raw_count, updated_at)
        VALUES (${institution}, 0, NULL, NULL, NOW(), NULL, ${newCount ?? null}, ${rawCount ?? null}, NOW())
        ON CONFLICT (institution) DO UPDATE SET
          consecutive_failures = 0,
          last_failure_reason = NULL,
          last_failure_at = NULL,
...
// ── FDA Designation Job health ─────────────────────────────────────────────
// Stored in scraper_health under the sentinel key "__fda_designation__" so the
// existing health infrastructure handles it without a new table.

const FDA_DESIGNATION_KEY = "__fda_designation__";

/** Persist the result of a completed FDA designation match run. */
export async function recordFdaDesignationHealth(tagged: number, errors: number): Promise<void> {
  try {
    if (errors === 0) {
      await db.execute(sql`
        INSERT INTO scraper_health
          (institution, consecutive_failures, last_failure_reason, last_failure_at, last_success_at,
           backoff_until, last_success_new_count, last_success_raw_count, updated_at)
        VALUES (${FDA_DESIGNATION_KEY}, 0, NULL, NULL, NOW(), NULL, ${tagged}, ${tagged}, NOW())
        ON CONFLICT (institution) DO UPDATE SET
          consecutive_failures = 0,
          last_failure_reason = NULL,
          last_failure_at = scraper_health.last_failure_at,
          last_success_at = NOW(),
          last_success_new_count = ${tagged},
          last_success_raw_count = ${tagged},
          updated_at = NOW()
      `);
    } else {
      await db.execute(sql`
        INSERT INTO scraper_health
          (institution, consecutive_failures, last_failure_reason, last_failure_at, last_success_at,
           backoff_until, last_success_new_count, last_success_raw_count, updated_at)
        VALUES (${FDA_DESIGNATION_KEY}, 1, ${"partial errors: " + errors}, NOW(), NULL, NULL, NULL, NULL, NOW())
        ON CONFLICT (institution) DO UPDATE SET
          consecutive_failures = scraper_health.consecutive_failures + 1,
          last_failure_reason = ${"partial errors: " + errors},
          last_failure_at = NOW(),
          updated_at = NOW()
      `);
    }
  } catch (err: any) {
    console.warn(`[scraperState] recordFdaDesignationHealth failed: ${err?.message}`);
  }
}

/** Load stored health for the FDA designation matcher. */
export async function loadFdaDesignationHealth(): Promise<ScraperHealthRow | null> {
  try {
    const result = await db.execute(
      sql`SELECT * FROM scraper_health WHERE institution = ${FDA_DESIGNATION_KEY} LIMIT 1`
    );
    const row = (result.rows as any[])[0];
    if (!row) return null;
    return {
    ...
    console.warn(`[scraperState] loadFdaDesignationHealth failed: ${err?.message}`);
    return null;
  }
}
