...
export const ucsdScraper: InstitutionScraper = {
  institution: INST,
  async scrape(): Promise<ScrapedListing[]> {
    console.log(`[scraper] ${INST}: fetching campus-SD listing pages...`);
    try {
      ...
      addResults(extractListings(html));
      ...
      for (let page = 2; page <= totalPages; page++) {
        ...
        try {
          ...
          if (!postRes.ok) {
            console.warn(`[scraper] ${INST}: page ${page} returned ${postRes.status}, stopping`);
            break;
          }
