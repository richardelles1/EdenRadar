...
const BASE = "https://tto.northwestern.edu";
  ...
  async scrape(): Promise<ScrapedListing[]> {
    try {
      ...
      for (const url of urls) {
        ...
        $("article, .views-row, .technology, .tech-card").each((_, el) => {
      ...
      console.log(`[scraper] Northwestern: ${results.length} listings`);
      return results;
    } catch (err: any) {
      console.error(`[scraper] Northwestern failed: ${err?.message}`);
      return [];
    }
  },
};
