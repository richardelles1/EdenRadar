  ...
  async scrape(): Promise<ScrapedListing[]> {
    try {
      ...
      for (const url of urls) {
        ...
        $("article, .views-row, .technology, .tech-listing, .post").each((_, el) => {
      ...
      console.log(`[scraper] UPenn: ${results.length} listings`);
      return results;
    } catch (err: any) {
      console.error(`[scraper] UPenn failed: ${err?.message}`);
      return [];
    }
  },
};
