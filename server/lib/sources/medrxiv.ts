...
async function fetchRecentPreprints(cursor = 0): Promise<any[]> {
  const today = new Date();
  const past = new Date();
  past.setDate(today.getDate() - 90);
  const from = past.toISOString().split("T")[0];
  const to = today.toISOString().split("T")[0];
  const url = `${BASE}/medrxiv/${from}/${to}/${cursor}/json`;
  const res = await fetch(url, { signal: AbortSignal.timeout(10000) });
  if (!res.ok) throw new Error(`medRxiv API error: ${res.status}`);
...
function toRawSignal(item: any): RawSignal {
  return {
    id: `medrxiv-${item.doi ?? item.rel_doi ?? Math.random()}`,
    ...
    url: item.doi
      ...
      : item.rel_link ?? "https://www.medrxiv.org",
...
export async function searchMedrxiv(query: string, maxResults = 10): Promise<RawSignal[]> {
  try {
    const items = await fetchRecentPreprints(0);
    const matched = items
      .filter((item) => matchesQuery(`${item.title ?? item.rel_title} ${item.abstract ?? item.rel_abs}`, query))
      .slice(0, maxResults);
    return matched.map(toRawSignal);
  } catch (err) {
    console.error("medRxiv search error:", err);
    return [];
  }
}
