...
export async function registerRoutes(
  ...
  // ── USPTO Patent Cross-Reference (ip_type + patent_status from ODP API) ────

  app.get("/api/admin/enrichment/uspto-crossref/count", requireAdmin, async (_req, res) => {
    try {
      const result = await db.execute<{ total: number; covered: number }>(sql`
        SELECT
          COUNT(*)::int AS total,
          COUNT(*) FILTER (
            WHERE institution IN (
              'Johns Hopkins University','University of Arizona','Purdue University',
              'University of Wisconsin','UCLA','Arizona State University',
              'Columbia University','UC Davis','University of Pittsburgh',
              'Cornell University','Stanford University','MIT','UC San Diego',
              'University of Minnesota','Northwestern University',
              'University of South Florida','Washington University in St. Louis',
              'UC Berkeley','UC Irvine','Rutgers University',
              'University of Illinois','Emory University','University of Washington',
              'Princeton University','University of Pennsylvania','Penn State University',
              'UC San Francisco','Rice University','University of Southern California',
      ...
      res.json({
        ...
        covered: result.rows[0].covered ?? 0,
      });
    } catch (err: any) {
      res.status(500).json({ error: err.message });
    }
  ...
  app.post("/api/admin/enrichment/uspto-crossref/spot-check", requireAdmin, async (_req, res) => {
    const apiKey = process.env.USPTO_ODP_API_KEY;
    if (!apiKey) {
      return res.status(400).json({ error: "USPTO_ODP_API_KEY environment variable is not set" });
    }
    try {
      const { runSpotCheck } = await import("./lib/pipeline/usptoPatentLookup");
      const results = await runSpotCheck(apiKey);
      res.json({ results });
    } catch (err: any) {
      res.status(500).json({ error: err.message });
    }
  ...
  app.post("/api/admin/enrichment/uspto-crossref/run", requireAdmin, async (_req, res) => {
    const apiKey = process.env.USPTO_ODP_API_KEY;
    if (!apiKey) {
      return res.status(400).json({ error: "USPTO_ODP_API_KEY environment variable is not set" });
    }
    try {
      const { crossReferenceAssets } = await import("./lib/pipeline/usptoPatentLookup");
      ...
      const rows = await db.execute<{
        ...
        WHERE relevant = true
          AND source_type = 'tech_transfer'
          AND (ip_type IS NULL OR ip_type IN ('unknown', ''))
          AND (human_verified->>'ipType') IS NULL
          AND institution IN (
            'Johns Hopkins University','University of Arizona','Purdue University',
            'University of Wisconsin','UCLA','Arizona State University',
            'Columbia University','UC Davis','University of Pittsburgh',
      ...
      const { matches, summary } = await crossReferenceAssets(assets, apiKey);

      // Write matches to DB — never overwrite human-verified fields
      let written = 0;
      for (const m of matches) {
        try {
          await db.execute(sql`
            ...
            SET
              ip_type = ${m.ipType},
              patent_status = ${m.patentStatus},
              enrichment_sources = COALESCE(enrichment_sources, '{}'::jsonb) ||
                '{"ip_type":"rule:uspto_patentsview","patent_status":"rule:uspto_patentsview"}'::jsonb
            WHERE id = ${m.assetId}
              AND (ip_type IS NULL OR ip_type IN ('unknown', ''))
              AND (human_verified->>'ipType') IS NULL
          `);
          ...
          console.warn(`[uspto-crossref] write failed for asset ${m.assetId}: ${e.message}`);
        }
      }

      console.log(
        `[uspto-crossref] Complete: ${written} written, ${summary.matched} matched, ` +
        `${summary.unmatched} unmatched, ${summary.skipped} skipped of ${summary.total} assets ` +
        `across ${summary.institutions} institutions`,
      );

      res.json({ ...summary, written });
    } catch (err: any) {
      console.error("[uspto-crossref] Error:", err.message);
      res.status(500).json({ error: err.message });
    }
  });
