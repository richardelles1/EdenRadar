...
export async function registerRoutes(
  ...
  let modalityFillResult: {
    total: number; ruleMatched: number; tierCounts: { t1: number; t2: number; t3: number };
    gptResolved: number; totalUpdated: number; unresolved: number;
  } | null = null;
  ...
  app.post("/api/admin/enrich/modality-fill", requireAdmin, async (req, res) => {
    try {
      ...
      (async () => {
        ...
        const { computeCompletenessScore } = await import("./lib/pipeline/contentHash");
        ...
        type AR = { id: number; asset_name: string; summary: string | null; abstract: string | null; indication: string | null; development_stage: string | null; mechanism_of_action: string | null; ip_type: string | null; patent_status: string | null; source_type: string | null };

        function applyRules(a: AR): string | null {
          const name = a.asset_name ?? "";
          const full = [name, a.summary ?? "", a.abstract ?? ""].join(" ").toLowerCase();
          for (const r of NAME_PRIORITY) {
            if (r.pattern.test(name)) {
              if (r.modality === "medical device") {
                if (/\binhibitor\b|\btherapy\b|\bdrug\b|\bcompound\b|\bantibody\b|\bvaccine\b|\bgene\b|\bpeptide\b|\bbiologic\b|\bmolecule\b/i.test(name)) continue;
              }
              return r.modality;
            }
          }
          for (const r of TIER1) { if (r.pattern.test(full)) return r.modality; }
          for (const r of TIER2) { if (r.pattern.test(full)) return r.modality; }
          for (const r of TIER3) { if (r.pattern.test(full)) return r.modality; }
          return null;
        }
        ...
        try {
          ...
          // Step 2: fetch unclassified
          const { rows } = await client.query<AR>(`
          ...
          const ruleMatches: Array<{ id: number; modality: string; asset: AR }> = [];
          const unmatched: AR[] = [];
          const tierCounts = { t1: 0, t2: 0, t3: 0 };

          for (const a of rows) {
            const m = applyRules(a);
            if (m) {
              ruleMatches.push({ id: a.id, modality: m, asset: a });
              const full = [a.asset_name, a.summary ?? "", a.abstract ?? ""].join(" ").toLowerCase();
              let tier = 3;
              for (const r of TIER1) { if (r.pattern.test(full)) { tier = 1; break; } }
              if (tier === 3) { for (const r of TIER2) { if (r.pattern.test(full)) { tier = 2; break; } } }
              if (tier === 1) tierCounts.t1++; else if (tier === 2) tierCounts.t2++; else tierCounts.t3++;
            } else {
              unmatched.push(a);
            }
          }
          ...
          // Step 3: GPT fallback
          const gptMatches: Array<{ id: number; modality: string; asset: AR }> = [];
          const openai = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });
          const GPT_BATCH = 50;

          for (let i = 0; i < unmatched.length; i += GPT_BATCH) {
            const batch = unmatched.slice(i, i + GPT_BATCH);
            const items = batch.map((a, idx) => `${idx + 1}. [ID:${a.id}] ${[a.asset_name, a.summary ?? ""].join(" ").slice(0, 500)}`);
            const prompt = `Classify each biotech asset into one modality from: ${CANONICAL_MODALITIES.join(", ")}.\nReply ONLY with JSON: [{"id": N, "modality": "..."},...]. Use "unknown" if truly unclear.\n\n${items.join("\n")}`;
            try {
              const resp = await openai.chat.completions.create({ model: "gpt-4o-mini", messages: [{ role: "user", content: prompt }], temperature: 0, response_format: { type: "json_object" } });
              const raw = resp.choices[0]?.message?.content ?? "{}";
              let parsed: any;
              try { parsed = JSON.parse(raw); } catch { continue; }
              const arr: Array<{ id: number; modality: string }> = Array.isArray(parsed) ? parsed : (parsed.results ?? parsed.assets ?? parsed.classifications ?? []);
              for (const item of arr) {
                if (typeof item.id === "number" && typeof item.modality === "string") {
                  ...
                  if (m !== "unknown" && CANONICAL_MODALITIES.includes(m)) {
                    const asset = batch.find(a => a.id === item.id);
                    if (asset) gptMatches.push({ id: item.id, modality: m, asset });
                  }
                }
              }
            } catch { /* continue */ }
            if (i + GPT_BATCH < unmatched.length) await new Promise(r => setTimeout(r, 200));
          }
          ...
          console.log(`[modality-fill] Done — updated ${totalUpdated} assets (T1:${tierCounts.t1} T2:${tierCounts.t2} T3:${tierCounts.t3} GPT:${gptMatches.length})`);
        } finally {
          client.release();
          await dbPool.end();
