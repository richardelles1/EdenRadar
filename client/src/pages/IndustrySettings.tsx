...
export default function IndustrySettings() {
  ...
  useEffect(() => {
    if (!isIndustry || !session?.access_token) {
  ...
  const alertPrefsChanged =
    JSON.stringify(alertPrefs.therapeuticAreas.slice().sort()) !== JSON.stringify((profile?.therapeuticAreas ?? []).slice().sort()) ||
    JSON.stringify(alertPrefs.modalities.slice().sort()) !== JSON.stringify((profile?.modalities ?? []).slice().sort()) ||
    JSON.stringify(alertPrefs.dealStages.slice().sort()) !== JSON.stringify((profile?.dealStages ?? []).slice().sort());
    ...
    <div className="max-w-5xl mx-auto px-5 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="space-y-6">
            ...
            <div className="space-y-5">
              <div className="flex items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">Receive a summary of new TTO assets matching your alerts</p>
                </div>
          ...
          <div className="rounded-xl border border-card-border bg-card p-5">
            <SectionHeader icon={Layers} title="Alert Preferences" description="Therapeutic focus, modalities, and stages used to score TTO assets" />
            {profileLoading ? (
              <div className="space-y-2">{[1, 2, 3].map((i) => <div key={i} className="h-4 rounded bg-muted animate-pulse w-3/4" />)}</div>
            ) : (
              <div className="space-y-5">
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {THERAPEUTIC_AREA_OPTIONS.map((area) => (
                      <ToggleChip key={area} label={area}
                  ...
                  {alertPrefs.therapeuticAreas.filter((a) => !THERAPEUTIC_AREA_OPTIONS.includes(a)).length > 0 && (
                    <div className="flex flex-wrap gap-1.5 mt-2">
                      {alertPrefs.therapeuticAreas.filter((a) => !THERAPEUTIC_AREA_OPTIONS.includes(a)).map((area) => (
                        <ToggleChip key={area} label={area} active
                          onClick={() => toggleAlertPref("therapeuticAreas", area)}
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {MODALITY_OPTIONS.map((m) => (
                      <ToggleChip key={m} label={m} active={alertPrefs.modalities.includes(m)}
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {STAGE_OPTIONS.map((s) => (
                      <ToggleChip key={s} label={s} active={alertPrefs.dealStages.includes(s)}
                ...
                <div className="flex items-center gap-3 pt-1">
                  <Button size="sm" onClick={saveAlertPrefs}
                    disabled={alertPrefsSaving || alertPrefsSaved || !alertPrefsChanged}
                    data-testid="button-save-alert-prefs" className="gap-1.5">
                    {alertPrefsSaved ? (
        ...
        <div className="space-y-6">
          <div className="rounded-xl border border-card-border bg-card p-5">
            <SectionHeader icon={Building2} title="Profile" description="Your company information and onboarding status" />
            {profileLoading ? (
              <div className="space-y-2">{[1, 2].map((i) => <div key={i} className="h-4 rounded bg-muted animate-pulse w-1/2" />)}</div>
            ) : profile ? (
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-x-4 gap-y-3 text-sm">
                  ...
                  <div>
                    ...
                    <span data-testid="text-onboarding-status"
